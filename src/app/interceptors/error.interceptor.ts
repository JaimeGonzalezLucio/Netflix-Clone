...
*/
  ...
  return next(req).pipe(
    catchError(err => {
      console.log(err);
      if (err.status==401) {
        alert('Necesitas iniciar sesión para acceder a esta página, agrega tu apikey: ' + err.error.detail);                
      }

      if (err.status==403) {
        alert('No tienes permisos para acceder a esta página, agrega tu apikey: ' + err.error.detail);        
      }

      if (err.status==404) {
        alert('No se ha encontrado la página solicitada: ' + err.error.detail);                
        window.location.href = '/home';
      }

      if (err.status==500) {
        alert('Error interno del servidor: ' + err.error.detail);   
      }

      if (err.status==0) {
        alert('Error de conexión: ' + err.error.detail);  
      }
